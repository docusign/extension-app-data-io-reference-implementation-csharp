...
You can configure Terraform to use a non-standard Docker daemon socket by setting the `DOCKER_HOST` environment variable or by assigning the `docker_host` Terraform variable. For example, if you are using Podman, you can set the `DOCKER_HOST` to point to the Podman socket:
