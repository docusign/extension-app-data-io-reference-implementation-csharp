locals {
  ...
  application_cloud_run_container_environment_variables = {
    ...
    ASPNETCORE_URLS                     = "http://+:${var.application_port}"
    AuthSettings__JwtSecretKey          = local.application_jwt_secret_key
    AuthSettings__OAuthClientId         = local.application_oauth_client_id
    AuthSettings__OAuthClientSecret     = local.application_oauth_client_secret
    AuthSettings__AuthorizationCode     = local.application_authorization_code
  }
