...
locals {
  ...
  application_jwt_secret_key      = var.application_jwt_secret_key != "" ? var.application_jwt_secret_key : one(module.generate_jwt_secret_key[*].random_bytes)
  application_oauth_client_id     = var.application_oauth_client_id != "" ? var.application_oauth_client_id : one(module.generate_oauth_client_id[*].random_bytes)
  application_oauth_client_secret = var.application_oauth_client_secret != "" ? var.application_oauth_client_secret : one(module.generate_oauth_client_secret[*].random_bytes)
  application_authorization_code  = var.application_authorization_code != "" ? var.application_authorization_code : one(module.generate_authorization_code[*].random_bytes)
